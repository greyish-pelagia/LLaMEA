...
class Algorithm:
    ...
    def _differential_evolution(self, max_evals: int):
        ...
        while self._remaining > end_remaining and pop_size >= 8 and self._best_y > 1e-10:
            ...
            for i in np.random.permutation(pop_size):
                ...
                f = -1.0
                for _ in range(12):
                    f = mu_f + 0.10 * np.random.standard_cauchy()
                    if f > 0.0:
                        break
                ...
                cr = float(np.clip(np.random.normal(mu_cr, 0.10), 0.0, 1.0))
            ...
            if success_f:
                ...
                # For CR, standard SHADE uses arithmetic mean or lehmer mean
                avg_cr = float(np.sum(weights * scr))
                
                memory_f[memory_pos] = lehmer_f
                memory_cr[memory_pos] = avg_cr
    ...
    def _adaptive_local_mutation(self, max_evals: int):
        ...
        while self._remaining > end_remaining and self._best_y > 1e-10:
            ...
            if r < 0.35:
                ...
                step = np.random.standard_cauchy(size=self.dim) * sigma * 0.15
    ...
    def _evolution_strategy(self, max_evals: int | None = None):
        ...
        while self._remaining - end_remaining >= pop_size and self._best_y > 1e-10:
            # BIPOP interleaving: alternate small and large populations
            if restart % 2 == 1:
                lam_base = max(10, int(0.5 * pop_size))
                lam = min(lam_base, 32, self._remaining - end_remaining)
            else:
                lam = min(max(pop_size, int(pop_size * (2.0 ** (restart // 2)))), 160, self._remaining - end_remaining)
