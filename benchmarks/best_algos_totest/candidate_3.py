...
class Algorithm:
    ...
    def _evolution_strategy(self, max_evals: int | None = None):
        ...
        while self._remaining - end_remaining >= pop_size and self._best_y > 1e-10:
            # IPOP growth: slowly increase population size. Small lam breaks covariance in 30D.
            lam = min(max(pop_size, int(pop_size * (1.4 ** restart))), 160, self._remaining - end_remaining)
